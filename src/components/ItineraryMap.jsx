...
/* =========================================================
   ...
   Supports full countries or localized Tokyo Barrios map.
   ========================================================= */
...
function FitAndLock({ hasJapan, hasKorea, isTokyoBarrios, points }) {
    const map = useMap()
    useEffect(() => {
        ...
        let bounds
        if (isTokyoBarrios && points && points.length > 0) {
            // Fit precisely to Tokyo barrios and excursion points with generous padding
            const latLngs = points.map(p => p.coords)
            bounds = L.latLngBounds(latLngs)
        } else if (hasJapan && hasKorea) {
        ...
        map.fitBounds(bounds, { padding: isTokyoBarrios ? [40, 40] : [30, 30] })
    ...
    }, [map, hasJapan, hasKorea, isTokyoBarrios, points])
    return null
}

export default function ItineraryMap({ chapters, activeCity, onCityClick, isTokyoBarrios = false }) {
    ...
    const active = chapters[activeCity] || chapters[0]
                ...
                <div className="td-detail-body">
                    <div className="td-detail-header">
                        ...
                        <div>
                            ...
                            {active.barrio && (
                                <span className="td-detail-barrio-tag">{active.barrio}</span>
                            )}
                    ...
                    {active.saborSugerido && (
                        <div className="td-detail-sabor">
                            <span className="td-detail-sabor-icon">🍜</span>
                            <div>
                                <strong>Sabor del día sugerido</strong>
                                <span>{active.saborSugerido}</span>
                            </div>
                        </div>
                    )}
